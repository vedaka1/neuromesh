...
class UserRequestRepository(BaseUserRequestRepository):
    ...
    async def delete_user_requests(self, user_id: uuid.UUID) -> None:
        ...
        await self.session.execute(query, {'value': user_id})
    ...
    async def get_by_user_and_model_name(self, model_name: str, user_id: uuid.UUID) -> UserRequest | None:
        query = text("""SELECT * FROM users_requests WHERE user_id = :user_id AND neural_network_name = :model_name;""")
        result = await self.session.execute(query, {'user_id': user_id, 'model_name': model_name})
        data = result.mappings().one_or_none()
        if not data:
            return None
    ...
    async def get_all_by_user_id(self, user_id: uuid.UUID, limit: int = 10, offset: int = 0) -> list[UserRequest]:
        query = text("""SELECT * FROM users_requests WHERE user_id = :user_id LIMIT :limit OFFSET :offset;""")
    ...
    async def update_user_requests(self, user_id: uuid.UUID, model_name: str, amount: int):
        query = text(
            """
        ...
        await self.session.execute(query, {'val': amount, 'id': user_id, 'model_name': model_name})
