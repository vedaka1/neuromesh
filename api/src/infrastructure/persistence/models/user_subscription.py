...
class UserSubscriptionModelDB(Base):
    ...
    subscription_name: Mapped[str] = mapped_column(ForeignKey('subscriptions.name', ondelete='CASCADE'))
    created_at: Mapped[datetime] = mapped_column(TIMESTAMP(timezone=False), server_default=func.now(), nullable=False)
