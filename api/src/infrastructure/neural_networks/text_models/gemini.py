...
def init_gemini() -> genai.GenerativeModel:
    gemini = genai
    gemini.configure(api_key=config.API_KEY_GEMINI)
...
class Gemini(BaseTextModel):
    ...
    async def generate_response(self, user_id: UUID, message: dict[str, Any]) -> str | None:
        try:
    ...
    def _test_access(cls) -> bool:
        try:
            ...
            logging.error('Gemini not available. Setting status to False')
            return False
