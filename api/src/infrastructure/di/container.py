...
class SettingsProvider(Provider):
    ...
    def openai_client(self) -> openai.AsyncOpenAI:
        return openai.AsyncOpenAI(api_key=config.chatgpt.API_KEY_CHATGPT)


class DatabaseConfigurationProvider(Provider):
    @provide(scope=Scope.REQUEST, provides=AsyncSession)
    async def provide_db_connection(self, session_factory: async_sessionmaker) -> AsyncGenerator[AsyncSession, None]:
...
class DatabaseAdaptersProvider(Provider):
    ...
    unit_of_work = provide(Commiter, provides=ICommiter)
    ...
    user_request_repository = provide(UserRequestRepository, provides=BaseUserRequestRepository)
    user_subscription_repository = provide(UserSubscriptionRepository, provides=BaseUserSubscriptionRepository)
    neural_network_subscription_repository = provide(
        NeuralNetworkSubscriptionRepository,
        provides=BaseNeuralNetworkSubscriptionRepository,
    )
    neural_network_repository = provide(NeuralNetworkRepository, provides=BaseNeuralNetworkRepository)
    subscription_repository = provide(SubscriptionRepository, provides=BaseSubscriptionRepository)
