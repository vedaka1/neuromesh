...
class Keyboards:
    def select_model(self, user_id: int, data: dict):
        buttons = [
            [types.InlineKeyboardButton(text=model['name'], callback_data=f"selectModel_{user_id}_{model["name"]}")]
            for model in data['models']
        ]
        return types.InlineKeyboardMarkup(inline_keyboard=buttons)

    def all_models(self, data: set):
        buttons = [[types.InlineKeyboardButton(text=model, callback_data=f'selectModel_{model}')] for model in data]
        return types.InlineKeyboardMarkup(inline_keyboard=buttons)

    def all_subscriptions(self, data: dict):
        buttons = [
            [types.InlineKeyboardButton(text=sub['name'], callback_data=f"selectSub_{sub["name"]}")] for sub in data
        ]
        return types.InlineKeyboardMarkup(inline_keyboard=buttons)
